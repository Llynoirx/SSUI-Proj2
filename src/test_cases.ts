...
// Base class and subclasses we use
...
import {Row, Row_debug} from "./Row.js";
import {Strut, Strut_debug } from "./Strut.js";
import {Spring, Spring_debug } from "./Spring.js";
...
function setupInputHandling(dragHnd? : (x: number, y: number) => void) {
    dragHandler = dragHnd;
...
let dragHandler : ((x: number, y: number) => void) | undefined = undefined;

// State of our simple dragging interaction
let interactionState : 'start' | 'dragging' = 'start';
        ...
        case 'start':
            switch (evt.type) {
                case 'mousedown':
                    ...
                    if (dragHandler) dragHandler(evt.offsetX, evt.offsetY);
...
let objToResize : DrawnObjectBase | undefined;

// Handler that we use to resize objects when we get a new drag position
function resizer(x: number, y:number) : void {
...
function pickFirstLevel(x:number, y:number) : DrawnObjectBase | undefined {
...
function testFilled(top : DrawnObjectBase) {
    for (let i = 0; i < 10; i++) {
...
function testIcon(top : DrawnObjectBase, x : number, y : number) {
    const icn1 : IconObject = new IconObject(x,y, 100,100, "./images/test_image.png");
    icn1.resizesImage = false;
    top.addChild(icn1);
    
    const icn2 : IconObject = new IconObject(x+50,y+100, 100,100, "./images/test_image.png");
    icn2.resizesImage = true;
    top.addChild(icn2);
}

function testText(top : DrawnObjectBase, x : number, y : number) {
    const fnt1 = "40px sans-serif";
    const fnt2 = "100px Brush Script MT";
    top.addChild(new TextObject_debug(x,y, "{Testing}: ÃWMjyz", 
                                        fnt1, {w:10,h:0},'blue'));
    top.addChild(new TextObject_debug(x+10,y+70, "{Test}:ÃMjz", 
                                        fnt2, {w:0,h:0},'black','stroke'));
}
 
//. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .

function testCol(top : DrawnObjectBase, x : number, y : number) {
    let xpos = x; let ypos = y
    const col1 : Column = new Column_debug(xpos,ypos);
    top.addChild(col1);
    for (let i = 0; i < 3; i++) {
    ...
    const col2 : Column = new Column_debug(xpos+100+10,ypos);
    top.addChild(col2);
    for (let i = 0; i < 3; i++) {
        let cl = blueishColors[(blueishColors.length-1-(i*4+25)) % blueishColors.length];
        let aChild = new FilledObject(i*25,i*50, 100-(i*10),100, cl);
        col2.addChild(aChild);
        let st : DrawnObjectBase = new Spring_debug('h');
        if (i === 1) st = new Strut_debug(10,30);
        col2.addChild(st);
    }
    col2.h = 3*100+30 + 50;  // size of fixed size children + 50;
    col2.wJustification = 'center';
    
    const col3 : Column = new Column_debug(xpos+200+20,ypos);
    top.addChild(col3);
    for (let i = 0; i < 3; i++) {
        ...
        let st : DrawnObjectBase = new Spring_debug('h');
        if (i === 0) st = new Strut_debug(10,40);
        col3.addChild(st);
    }
...
function testRow(top : DrawnObjectBase, x : number, y : number) {
    let xpos = x; let ypos = y
    const r1 : Row = new Row_debug(xpos,ypos);
    top.addChild(r1);
    for (let i = 0; i < 3; i++) {
        let cl = blueishColors[(blueishColors.length-1-(i*4+15)) % blueishColors.length];
        let aChild = new FilledObject(i*25,i*50, 100-(i*10),100, cl);
...
function testRowCol(top : DrawnObjectBase, x : number, y : number) {
    const r1 : Row = new Row_debug(x,y);
    let clcnt = 0;
    top.addChild(r1);
    for (let i = 0; i < 3; i++) {
        if (i == 1) {
            let achild : Column = new Column_debug(0,0,40,200);
...
function testErr() {
    ...
    try { throw new Error("My error message 1");} catch(e) { Err.handle(e); }
    Err.handleMethod = 'message';
    try { throw new Error("My error message 2");} catch(e) { Err.handle(e,"(extra)"); }
    Err.handleMethod = 'full_message';
    try { throw new Error("My error message 3");} catch(e) { Err.handle(e); }
    Err.handleMethod = 'throw';
    try { throw new Error("My error message 4");} catch(e) { Err.handle(e); }
}


//. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .

function testDrawableImage(ctx : DrawContext, x : number, y : number) {
    function notif(evt: Event, img:DrawableImage) {
        console.log("notified; drawing...");
        if (img.canvasImage) ctx.drawImage(img.canvasImage, x,y);
    }
